...
export async function generateOpenAIResponseAction(userQuery: string, sources: SelectSource[]): Promise<ActionState<StreamableValue<any, any>>> {
  try {
    const stream = createStreamableValue();

    const sourcesContext = sources.map((r, i) => 
      `Source ${i + 1}: ${r.title}\nURL: ${r.url}\nSummary: ${r.summary}\nText: ${r.text}\n`
    ).join("");
